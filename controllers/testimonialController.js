...
export const addTestimonials = (req, res) => {
  ...
  try {
    ...
    const addTestimonialsQuery =
      'INSERT INTO testimonials(testimonial_id,customer_name,email,content,rating) VALUES (?,?,?,?,?)';
    //storing the data
    ...
    connection.query(
      addTestimonialsQuery,
      addTestimonialsValue,
      (error, results) => {
        if (error) {
          console.log(error);
          res.status(500).json({
...
export const deleteTestimonials = (req, res) => {
  ...
  try {
    ...
    connection.query(checkTestiIdQuery, [testId], (checkError, checkResult) => {
      ...
      const deleteTestimonialsQuery =
        'DELETE FROM testimonials WHERE testimonial_id = ?';
      connection.query(
        deleteTestimonialsQuery,
        [testId],
        (deleteError, results) => {
          if (deleteError) {
...
export const updateTestimonials = (req, res) => {
  ...
  try {
    ...
    connection.query(checkTestiIdQuery, [testId], (checkError, checkResult) => {
      ...
      const updateTestimonialsQuery =
        'UPDATE testimonials SET customer_name = ?, email = ?, content = ?, rating = ? WHERE testimonial_id = ?';
      ...
      connection.query(
        updateTestimonialsQuery,
        updateTestimonialsValue,
        (updateError, results) => {
          if (updateError) {
            console.log(error);
            res.status(500).json({
